...
// Home displays the virtual home page
...
func (app *application) PaymentSucceeded(w http.ResponseWriter, r *http.Request) {
	...
	txn := models.Transaction{
		...
		TransactionStatusID: clearedTransactionStatusID,
	}
...
func (app *application) BronzePlan(w http.ResponseWriter, r *http.Request) {
	bronzePlanID := 2
	widget, err := app.DB.GetWidget(bronzePlanID)
	if err != nil {
		app.errorLog.Println(err)
		return
	}
...
func (app *application) ShowResetPassword(w http.ResponseWriter, r *http.Request) {
	incomingURL := r.RequestURI
	matchingURL := fmt.Sprintf("%s%s", app.config.frontend, incomingURL)

	signer := urlsigner.Signer{
		Secret: []byte(app.config.secretKey),
	}

	isValid := signer.VerifyToken(matchingURL)
